 ...
 * Roche 朋友圈插件 v2.17.0
 * 完全拟真微信朋友圈的沉浸式模拟
 * v2.17.0: 总提示词聊天分类移除重复的「user 认知行（memSync）」预览（保留可编辑的 user 认知行）；每个功能分类标题旁新增「约 N tokens」实时 token 估算（按完整实际提示词估算，改后立即更新）。
...
(function () {
  ...
  // 粗略估算 token 数（CJK 按 1.5/字、英文单词/数字/符号按 1.3/个）
  function estimateTokens(text) {
    var s = String(text || '');
    if (!s) return 0;
    var cjk = (s.match(/[\u4e00-\u9fff\u3040-\u30ff\uac00-\ud7af]/g) || []).length;
    var rest = s.replace(/[\u4e00-\u9fff\u3040-\u30ff\uac00-\ud7af]/g, ' ');
    var words = (rest.match(/[A-Za-z0-9_]+|[^\s]/g) || []).length;
    ...
    switch (act) {
      ...
      case 'preview-feature-prompt': {
        ...
        previewFeaturePrompt(space, scF, keyF).then(function (txt) {
          ...
          state.contentPreview = { title: '实际发送的完整提示词（约 ' + estimateTokens(txt) + ' tokens）— ' + (labelsF[keyF] || keyF) + ' · ' + scF.charName, hint: '以下是该功能请求 AI 时实际拼成的提示词原文（变量已按当前数据替换；「聊天时 char 的提示词」与 chatContextProvider 逐项一致）。', text: txt };
          render();
...
+ '.' + ROOT_CLASS + ' .moments-group-tokens{margin-left:auto;font-size:11px;color:#666;background:#f0f2f5;border-radius:10px;padding:1px 8px;flex-shrink:0;}'
+ '.' + ROOT_CLASS + '.dark .moments-group-tokens{color:#aaa;background:#2A2A2A;}'
...
+ '.' + ROOT_CLASS + '.dark .moments-sheet-item.danger svg{color:#FF6B6B;}';
  ...
  function composeFeaturePrompt(space, sc, key) {
    if (!space || !sc) return '（未选择 char）';
    ...
    if (key === 'chat') {
      ...
      return buildPromptPreviewText(space, sc);
    }
    if (key === 'post') {
      var postCount = parseInt(sc.postCount, 10); if (isNaN(postCount) || postCount < 1) postCount = 1; if (postCount > 9) postCount = 9;
      var sys = '你是「' + sc.charName + '」，此刻正在刷微信朋友圈。\n';
    ...
    if (key === 'npc') {
      ...
      return buildNpcCommentSys(space, postN, sc, getCharNpcs(sc), []);
    }
    if (key === 'summary') {
      ...
      return '（总结范围：第 ' + from + '-' + to + ' 条' + (sc.includeComments === false ? '' : '，含评论') + '）\n\n' + prompt;
    }
    if (key === 'sync') {
      var tpl = (sc.syncPrompt || '').trim() || DEFAULT_SYNC_PROMPT;
      return '（记忆同步提示词，{charName} 已替换）\n\n' + tpl.replace(/\{charName\}/g, sc.charName);
    }
    if (key === 'gen') {
      var lines = [];
  ...
  function previewFeaturePrompt(space, sc, key) {
    return Promise.resolve(composeFeaturePrompt(space, sc, key));
  }
  ...
  function renderAllPromptsModal(space) {
    ...
    groups.forEach(function (g) {
      ...
      var tokEst = sc ? estimateTokens(composeFeaturePrompt(space, sc, g.key)) : 0;
      html += '<div class="moments-group">';
      html += '<div class="moments-group-hd" data-action="toggle-allprompt-group" data-group="' + escapeHtml(g.key) + '"><div class="moments-group-hd-row"><span class="moments-group-caret">' + (isCollapsed ? '▸' : '▾') + '</span><span class="moments-group-title">' + escapeHtml(g.label) + '</span><span class="moments-group-tokens">约 ' + tokEst + ' tokens</span></div><div class="moments-group-desc">' + escapeHtml(g.desc) + '</div></div>';
      if (!isCollapsed) {
